...
// Copilot review (resume-agent-web#24): the initial version hardcoded "Sunny"
// and always used the plural "projects", breaking both forkability (this app
// is fork-ready for other candidates) and grammar for a single remaining project.
describe('remainingProjectsFollowup / isRemainingProjectsFollowup (dynamic name + grammar)', () => {
  it('uses the given first name, not a hardcoded one', () => {
    expect(remainingProjectsFollowup('Alex', 4)).toBe("What are Alex's other 4 projects?")
  })

  it('uses singular "project" for a count of exactly 1', () => {
    expect(remainingProjectsFollowup('Sunny', 1)).toBe("What are Sunny's other 1 project?")
  })

  it('uses plural "projects" for counts other than 1', () => {
    expect(remainingProjectsFollowup('Sunny', 4)).toBe("What are Sunny's other 4 projects?")
    expect(remainingProjectsFollowup('Sunny', 0)).toBe("What are Sunny's other 0 projects?")
  })

  it('matches text generated by remainingProjectsFollowup, singular or plural', () => {
    expect(isRemainingProjectsFollowup("What are Sunny's other 4 projects?", 'Sunny')).toBe(true)
    expect(isRemainingProjectsFollowup("What are Sunny's other 1 project?", 'Sunny')).toBe(true)
  })

  it('does not match a different first name', () => {
    expect(isRemainingProjectsFollowup("What are Alex's other 4 projects?", 'Sunny')).toBe(false)
  })

  it('does not match unrelated text', () => {
    expect(isRemainingProjectsFollowup("Tell me about Sunny's other work", 'Sunny')).toBe(false)
  })
})
