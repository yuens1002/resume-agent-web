...
/**
 * The "show more" follow-up chip text for the deterministic "Show recent
 * work" flow (App.tsx) — parameterized by the candidate's first name (this
 * app is fork-ready for other candidates, per resume-agent's README) and
 * grammatically correct for a single remaining project ("1 project?", not
 * "1 projects?").
 */
export function remainingProjectsFollowup(firstName: string, count: number): string {
  return `What are ${firstName}'s other ${count} project${count === 1 ? '' : 's'}?`
}

/** Matches text generated by `remainingProjectsFollowup` for the given first name, any count, singular or plural. */
